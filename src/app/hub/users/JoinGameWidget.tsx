...
import { Participant } from "@/types/auth";
...
function Step2({
  ...
  const [countriesCache, setCountriesCache] = useState<Record<string, string[]>>({});
  ...
  useEffect(() => {
    ...
    const region = games.find((g: Game) => g.id === Number(selectedGame))?.location.toLowerCase() || "";
    if (countriesCache[region]) {
      setCountries(countriesCache[region]);
    }
  }, [selectedGame, games, countriesCache]);
  ...
  useEffect(() => {
    ...
    fetch(
      ...
      .then((data: CountryAPIResponse[]) => {
        ...
        setCountriesCache((prev) => ({ ...prev, [region]: countryList }));
        setCountries(countryList);
        setLoading(false);
      })
      .catch(() => setLoading(false));
  }, [selectedGame, games, countriesCache]); // <-- add countriesCache here
  ...
  const handleFinish = async () => {
    ...
    try {
      ...
      if (data.success && data.user) {
        ...
        const hasActive = Array.isArray(data.user.participants)
          ? data.user.participants.some((p: Participant) => p.participantStatus === "PENDING" || p.participantStatus === "ACTIVE")
          : false;
        useUserStore.getState().setIsRegistered(hasActive);
  ...
  return (
    <>
      ...
      <div
        ...
        style={{ opacity: isRegistered ? 0.3 : 1, transition: "opacity 0.3s" }}
      >
