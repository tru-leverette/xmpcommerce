...
export default function AdminDashboard() {
  ...
  const verifyAdminAccess = useCallback(async () => {
    try {
      ...
      // Check cache first to prevent duplicate verification calls
      const cacheKey = 'admin_verification_' + Date.now().toString().slice(0, -5) // 5-minute cache
      ...
      sessionStorage.setItem(cacheKey, 'verified')
  ...
  const handleGameInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
    ...
    if (name === 'location' && value && occupiedContinents.has(value)) {
  ...
  const validateGameForm = (form: typeof gameForm) => {
    ...
    } else if (occupiedContinents.has(form.location)) {
      newErrors.location = `${form.location} is currently occupied by an active game. Please select a different continent.`
    }
    ...
    try {
      ...
      const response = await fetch(`/api/games/${gameId}`, {
        ...
        body: JSON.stringify({
          ...
          location: editGameForm.location,
            ...
            <div className="border-b border-gray-200">
              <nav className="-mb-px flex space-x-8">
                ...
                <button
                  ...
                  className={`py-2 px-1 border-b-2 font-medium text-sm ${activeTab === 'games'
                    ? 'border-blue-500 text-blue-600'
                    : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'
                    }`}
                >
                ...
                <div className="bg-white rounded-lg shadow p-6">
                  ...
                  <div className="grid md:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      {gameErrors.location && (
                        <p className="text-red-500 text-sm mt-1">{gameErrors.location}</p>
                      )}
                    </div>
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                            {editingGame === game.id ? (
                              <div>
                                ...
                                {editGameErrors.location && (
                                  <p className="text-red-500 text-xs mt-1">{editGameErrors.location}</p>
                                )}
                              </div>
                            ) : (
                              game.location
                      ...
                      <tr key={activity.id}>
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="flex items-center">
                            <div className={`w-2 h-2 rounded-full mr-3 ${activity.type === 'GAME_CREATED' ? 'bg-green-500' :
                        ...
                        {Array.from({ length: Math.min(5, activityTotalPages) }, (_, i) => {
                          ...
                          return (
                            <button
                              ...
                              className={`relative inline-flex items-center px-4 py-2 border text-sm font-medium ${pageNum === activityPage
